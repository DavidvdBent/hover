...
import { ArrowRight, Award, Clock, Edit, Euro, GraduationCap, Star } from 'lucide-react'
...
const AllCourses = async({query, currentPage} : {
  ...
  return (
    <div className="grid grid-cols-2 gap-8 pt-8 mx-6 justify-around">
        {allCourses?.map((course  :any, index : any)=> (
            <div key={index} className="border-2 m-auto  max-w-[500px] rounded-lg p-8 shadow-md mb-8">
                <Image src={course.url} height={500} width={500} alt='' className="rounded-lg mx-auto"/>
                <h2 className="mt-4 font-semibold text-lg text-center">{course.name}</h2>
                <p className="mt-2">{course.info.substring(0,175)}...</p>
                <div className='flex gap-4 justify-center'>
                ...
                <div className="flex justify-between mt-10">
                    <div className=" flex gap-2 items-center">
                        <Clock className="text-gray-500 h-5 w-5"/>
                        <p className="text-gray-500"> {course.hours}</p>
                    </div>
                    <div className=" flex gap-2 items-center">
                        <GraduationCap className="text-gray-500 h-6 w-6"/>
                        {/* Make THis DYNMIC! */}
                    ...
                    <div className=" flex gap-[1px] items-center">
                        <Euro className="text-gray-500 h-4 w-4"/>
                        {/* Make THis DYNMIC! */}
